...
Status TablePlugin::HandleRequest(const api::TableParameters &params, util::json::Object &result)
{
            ...
            &&(time_limits.second > params.time_period_from))
    {
        ...
        for (int k = 0; k < our_sources.size(); ++k) {
        ...
        for (size_t index_source = 0; index_source < our_sources.size(); ++index_source)
        {
            for (size_t index_destination = 0; index_destination < our_destinations.size(); ++index_destination)
            {
                       ...
                       snapped_phantoms[our_destinations[index_destination]].reverse_segment_id.id) // one way
                {
                    ...
                    part_of_edge_weight = abs(snapped_phantoms[our_sources[index_source]].GetForwardWeightPlusOffset()
                ...
                if(raw_route.is_valid()) {
                    ...
                    facade.GetUncompressedGeometry(snapped_phantoms[our_sources[index_source]].reverse_packed_geometry_id, source_geometry);
                    osm_node_start.first = facade.GetOSMNodeIDOfNode(source_geometry[source_geometry.size() - snapped_phantoms[our_sources[index_source]].fwd_segment_position - 1]);
                    facade.GetUncompressedGeometry(snapped_phantoms[our_sources[index_source]].forward_packed_geometry_id, source_geometry);
                    osm_node_start.second = facade.GetOSMNodeIDOfNode(source_geometry[snapped_phantoms[our_sources[index_source]].fwd_segment_position]);

                    std::vector<NodeID> target_geometry;
                    facade.GetUncompressedGeometry(snapped_phantoms[our_destinations[index_destination]].reverse_packed_geometry_id, target_geometry);
                    osm_node_finish.first = facade.GetOSMNodeIDOfNode(target_geometry[target_geometry.size() - snapped_phantoms[our_destinations[index_destination]].fwd_segment_position - 1]);
                    facade.GetUncompressedGeometry(snapped_phantoms[our_destinations[index_destination]].forward_packed_geometry_id, target_geometry);
                    osm_node_finish.second = facade.GetOSMNodeIDOfNode(target_geometry[snapped_phantoms[our_destinations[index_destination]].fwd_segment_position]);
                    ...
                    if(legGeometry.osm_node_ids.size() > 2){
                        ...
                        legGeometry.osm_node_ids[legGeometry.osm_node_ids.size() - 1] =
                                (legGeometry.osm_node_ids[legGeometry.osm_node_ids.size() -2] == osm_node_finish.second) ?
                                osm_node_finish.first : osm_node_finish.second;
                    }
                    //volatile auto tmp = facade.GetAdjacentEdgeRange(source_geometry.front());

                    // Check every edge (a pair of nodes)
                    if((legGeometry.annotations.size() == legGeometry.osm_node_ids.size()-1)  // each edge(pair of nodes) has a weight
                        && !part_of_edge_weight){
                        for (unsigned curr_node = 0; curr_node < legGeometry.annotations.size() /*legGeometry.GetNumberOfSegments()*/; ++curr_node) {
//                            if (legGeometry.osm_node_ids[curr_node] == legGeometry.osm_node_ids[curr_node+1])
//                                continue;
                            ...
                            for (unsigned i = 0; i < started_time.size(); ++i) {
                                ...
                                if (has_addition_time) {
                                    while ((addition_time_from != addition_time_to)
                                        ...
                                        else result_time[i] += addition_time_from->speed
                    ...
                    if(!part_of_edge_weight) {
                        ...
                        current_result = (api::TableAPI::AdditionalWeights){started_time[0], started_time[0] + (delta_time - 60*10), result_time[0]};
                        for (unsigned i = 1; i < started_time.size(); ++i) {
                            result_time[i] -= started_time[i];
                            if (current_result.additional_weight == result_time[i]) { // TODO better to check using correction like r>=w && r<w+correction
